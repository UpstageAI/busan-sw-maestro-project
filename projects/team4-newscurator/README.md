...
- 언론사 RSS 상태 확인
- 기사 필터링 및 중복 제거
- 룰 기반 후보 압축 후 Upstage 에이전트 선별 및 요약
...
1. 공개 RSS에서 최신 기사 목록을 수집합니다.
2. 포함/제외 키워드와 관심 분야로 1차 룰 기반 필터링을 수행합니다.
3. URL과 제목 유사도를 기준으로 중복 기사를 제거합니다.
4. 키워드 매칭, 최신성, 설명문 존재 여부로 룰 기반 우선순위 점수를 계산합니다.
5. 상위 20~30개 후보만 Upstage에 전달해 최종 기사 선별, 이슈 그룹화, 요약을 한 번에 수행합니다.

이 구조는 모든 기사에 LLM을 호출하지 않고, 키워드 기반 후보군 안에서 에이전트가 최종 브리핑 대상을 판단하도록 설계되어 토큰 비용과 응답 속도를 함께 관리합니다.
